        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            @if (session('status') === 'guardian-created' && session('temporaryPassword'))
                <div class="bg-yellow-50 border border-yellow-300 text-yellow-900 rounded-md p-4 text-sm">
                    {{ __('Guardian account created. Temporary password (share this securely, it will not be shown again):') }}
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <form method="GET" class="flex gap-2">
                        <x-text-input type="search" name="search" value="{{ request('search') }}" placeholder="{{ __('Search guardians...') }}" class="w-64" />
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        <thead>
                            <tr class="text-left text-gray-500">
                                ...
                                <th class="py-2 pr-4">{{ __('Children') }}</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($guardians as $guardian)
                                <tr>
                                    ...
                                    <td class="py-3 pr-4 text-right space-x-3">
                                        <a href="{{ route('admin.guardians.edit', $guardian) }}" class="text-indigo-600 hover:text-indigo-900">{{ __('Edit') }}</a>
                                        <form method="POST" action="{{ route('admin.guardians.toggle-status', $guardian) }}" class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="text-gray-600 hover:text-gray-900">
                                                {{ $guardian->status->value === 'active' ? __('Deactivate') : __('Activate') }}
                                            </button>
                                        </form>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="py-6 text-center text-gray-500">{{ __('No guardians found.') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
